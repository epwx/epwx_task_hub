...
const DailyClaimsTable: React.FC<DailyClaimsTableProps> = ({ claims, isAdmin = false, onDistribute, marking }) => (
  <div className="w-full overflow-x-auto rounded border border-slate-200 shadow-sm dark:border-slate-800">
    <table className={`w-full bg-white text-xs dark:bg-slate-900 sm:text-sm ${isAdmin ? 'min-w-[760px]' : 'min-w-[520px]'}`}>
      <thead className="bg-slate-200 dark:bg-slate-800">
        <tr>
          ...
          {isAdmin && <th className="min-w-[140px] py-2 px-2 text-left text-slate-700 dark:text-slate-200 sm:px-4">IP</th>}
      ...
      <tbody>
        {claims.map((claim) => (
          <tr key={claim.id} className="align-top border-b border-slate-200 last:border-none dark:border-slate-800">
            <td className="whitespace-nowrap bg-white py-2 px-2 font-mono text-slate-900 dark:bg-slate-900 dark:text-slate-100 sm:px-4">{isAdmin ? claim.wallet : formatWalletAddress(claim.wallet)}</td>
            ...
            {isAdmin && <td className="whitespace-nowrap bg-white py-2 px-2 text-slate-900 dark:bg-slate-900 dark:text-slate-100 sm:px-4">{claim.ip}</td>}
            {isAdmin && (
