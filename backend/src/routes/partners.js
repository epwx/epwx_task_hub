...
import { PartnerReferral, User, Partner } from '../models/index.js';

// Set up multer storage for verification images
  ...
  try {
    ...
    if (!name || !walletAddress) {
      return res.status(400).json({
        success: false,
        message: 'Name and wallet address are required'
      });
    }
    ...
    if (!req.file) {
      return res.status(400).json({
        success: false,
        message: 'Twitter followers screenshot is required for verification'
      });
    }
    ...
    res.json({
      success: true,
      message: 'Partner registered successfully. Awaiting admin verification.',
      partner: {
        id: partner.id,
        name: partner.name,
...
/**
 * POST /partners/:partnerId/generate-link
  ...
  try {
    ...
    res.json({
      ...
      referral: {
        ...
        referralCode: referral.referralCode,
        totalClaimsEarned: referral.totalClaimsEarned
      }
    });
  } catch (error) {
...
/**
 * GET /partners/:partnerId/earnings
...
router.get('/:partnerId/earnings', async (req, res) => {
  try {
    ...
    const earnings = await getPartnerEarningsHistory(
      partnerId,
      parseInt(limit),
      parseInt(offset)
    );
  ...
  try {
    ...
    if (!isAdminWallet(adminWallet)) {
      return res.status(403).json({
        success: false,
        message: 'Only admins can update partner status'
      });
    }
...
/**
 * GET /partners/admin/pending
 * Get pending partners for admin review (admin only)
 */
router.get('/admin/pending', async (req, res) => {
  try {
    ...
    if (!isAdminWallet(adminWallet)) {
      return res.status(403).json({
        success: false,
        message: 'Only admins can view pending partners'
      });
    }
    ...
    res.json({
      success: true,
      partners: pendingPartners.map(p => ({
        id: p.id,
        name: p.name,
        walletAddress: p.walletAddress,
        status: p.status,
        verificationImagePath: p.verificationImagePath,
    ...
    const partner = await updatePartnerStatus(partnerId, status);
